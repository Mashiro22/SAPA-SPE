...
class WSI_OTSurv_Dataset(Dataset):
    ...
    def __init__(self,
                 ...
                 sample_list=None,
                 instance_num=3000,
                 bag_sample_strategy='random',
                 **kwargs):
        """
        Args:
        """
        self.feature_len = instance_num
        self.bag_sample_strategy = bag_sample_strategy
    ...
    def init_df(self, df, data_source, target_transform, sample_col, slide_col,
        ...
        self.bag_size = int(bag_size)  # -1 keeps all patches
    ...
    def validate_survival_dataset(self):
        ...
        # Multi-slide cases are concatenated into one bag, so censorship must
        # also be consistent across every slide belonging to the case.
        num_unique_censoring = self.data_df.groupby(self.sample_col)[self.censorship_col].unique().apply(len)
        try:
            assert (num_unique_censoring == 1).all()
        except AssertionError:
            print('Each case_id must have only one unique censorship value.')
            raise
    ...
    @staticmethod
    def _spatial_uniform_indices(coords, bag_size, rng):
        n = len(coords)
        if n <= bag_size:
            return np.arange(n)

        coords = np.asarray(coords, dtype=np.float32)
        if coords.ndim != 2 or coords.shape[1] < 2 or np.allclose(coords[:, :2], 0):
            return rng.choice(n, bag_size, replace=False)

        xy = coords[:, :2]
        xy_min = xy.min(axis=0, keepdims=True)
        xy_max = xy.max(axis=0, keepdims=True)
        scale = np.maximum((xy_max - xy_min).max(), 1e-6)
        xy_norm = (xy - xy_min) / scale

        grid = int(np.ceil(np.sqrt(bag_size)))
        cell_xy = np.floor(np.clip(xy_norm, 0.0, 1.0 - 1e-8) * grid).astype(np.int64)
        cell_id = cell_xy[:, 0] * grid + cell_xy[:, 1]

        selected = []
        for cid in rng.permutation(np.unique(cell_id)):
            members = np.flatnonzero(cell_id == cid)
        ...
        if len(selected) < bag_size:
            ...
            remaining = np.array([i for i in range(n) if i not in selected_set], dtype=np.int64)
            # Space-filling fallback: sort by grid cell then coordinates and take evenly spaced patches.
            order = np.lexsort((xy_norm[remaining, 1], xy_norm[remaining, 0], cell_id[remaining]))
            remaining = remaining[order]
            need = bag_size - len(selected)
            take_pos = np.linspace(0, len(remaining) - 1, need, dtype=np.int64)
            selected.extend(remaining[take_pos].tolist())

        return np.asarray(selected[:bag_size], dtype=np.int64)
    ...
    def __getitem__(self, idx):
        ...
        for feat_path in feat_paths:
            if self.use_h5:
                with h5py.File(feat_path, 'r') as f:
                    features = f['features'][:]
                    coords = f['coords'][:] if 'coords' in f else np.zeros((features.shape[0], 2), dtype=np.float32)
            else:
        ...
        # Deterministic sampled-bag training. Evaluation keeps full bags when bag_size <= 0.
        if self.bag_size is not None and self.bag_size > 0 and len(all_features) > self.bag_size:
            seed_key = f"{self.get_sample_id(idx)}::{self.bag_size}"
            seed = int(hashlib.md5(seed_key.encode("utf-8")).hexdigest()[:8], 16)
            rng = np.random.default_rng(seed)
            if self.bag_sample_strategy == 'spatial_uniform':
                indices = self._spatial_uniform_indices(all_coords, self.bag_size, rng)
            else:
                indices = rng.choice(len(all_features), self.bag_size, replace=False)
