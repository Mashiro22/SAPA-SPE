...
def nll_loss(logits, y, c, alpha=0.0, eps=1e-7, reduction='mean'):
    ...
    # Match the official PTCMIL implementation and accept either [B] or
    # [B, 1] labels/censoring indicators.
    y = y.long().view(-1, 1)
    c = c.long().view(-1, 1)
...
def margin_capped_partial_ll_loss(lrisks, survival_times, event_indicators, margin=1.0):
    """
    One-sided margin-capped Cox partial likelihood.

    For an event sample i and a risk-set sample j, d_ij = r_j - r_i.
    Correct pairs with d_ij <= -margin are capped at -margin, so the loss
    stops rewarding extra risk-scale growth after the ordering is good enough.
    Wrong pairs remain uncapped and keep the original Cox penalty.
    """
    num_uncensored = torch.sum(event_indicators, 0)
    if num_uncensored.item() == 0:
        zero = torch.sum(lrisks) * 0
        return {'loss': zero, 'cox_uncapped_loss': zero, 'cox_margin_capped_loss': zero}
    ...
    uncapped_losses = []
    margin = float(margin)
    for i in range(lrisks.shape[0]):
        if event_indicators[i] <= 0:
            continue
        risk_set = lrisks[:i + 1]
        diff = risk_set - lrisks[i]
        capped_diff = torch.clamp(diff, min=-margin)
        losses.append(torch.logsumexp(capped_diff, dim=0))
        uncapped_losses.append(torch.logsumexp(diff, dim=0))

    if len(losses) == 0:
        zero = torch.sum(lrisks) * 0
        return {'loss': zero, 'cox_uncapped_loss': zero, 'cox_margin_capped_loss': zero}

    capped_loss = torch.stack(losses).sum() / num_uncensored
    uncapped_loss = torch.stack(uncapped_losses).sum() / num_uncensored
    return {
        'loss': capped_loss,
        'cox_uncapped_loss': uncapped_loss,
        'cox_margin_capped_loss': capped_loss,
    }
...
class MarginCappedCoxLoss(nn.Module):
    """
    Cox partial likelihood with one-sided pair margin capping.
    """
    ...
    def __call__(self, logits, times, censorships):
        return margin_capped_partial_ll_loss(
            lrisks=logits,
